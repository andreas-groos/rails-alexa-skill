...
RSpec.describe "alexa bad request", :type => :request do
  fixtures :users

  it "rejects an improperly signed request for ask action" do
    AlexaVerifier.configure { |config| config.enabled = true } if Rails.env.test?
    ...
    slots = {
      "recipename"=>{"name"=>"recipename", "value"=>"guacamole", "confirmationStatus"=>"NONE"},
      "ingredient"=>{"name"=>"ingredient", "value"=>"salt", "confirmationStatus"=>"NONE"}
    }
    ...
    AlexaVerifier.configure { |config| config.enabled = false } if Rails.env.test?
  end
